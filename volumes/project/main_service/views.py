...
from .serializers import VgSalesSerialier
...
class ShowGamesByRanking(APIView):
    ...
    def get(self, request, rank):
        #example : GET: http://127.0.0.1:8000/gamesRank/100

        temp = vgSales.objects.filter(Rank=rank)
...
class ShowGamesByName(APIView):
    ...
    def get(self, request, name):
        #ecample : GET: http://127.0.0.1:8000/gamesName/Mario
...
class NbestGameinEveryYear(APIView):
    ...
    def get(self, request):
        ...
        for w in re_which:
            if which == "Platform":
                all_vgSales = vgSales.objects.filter(Platform=w).order_by('Rank')
            elif which == "Genre":
                all_vgSales = vgSales.objects.filter(Genre=w).order_by('Rank')
            elif which == "Year":
                if w != 'N/A':
                    all_vgSales = vgSales.objects.filter(Year=w).order_by('Rank')
            t = all_vgSales[:int(n)]
            for all in t:
...
class FiveBestGameForOnePlatformInParticularYear(APIView):
    ...
    def get(self, request):
        ...
        games = vgSales.objects.filter(Platform=platform).filter(Year=year).order_by('Rank')
